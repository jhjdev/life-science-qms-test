...
router.get('/health', healthCheckLimiter, async function (req, res, next) {
  ...
  try {
    ...
    const membershipCount =
      await sql`SELECT COUNT(*) as count FROM group_members`;

    // Get recent activity
    const recentUser =
      await sql`SELECT name, created_at FROM users ORDER BY created_at DESC LIMIT 1`;
    const recentGroup =
      await sql`SELECT name, updated_at FROM groups ORDER BY updated_at DESC LIMIT 1`;
    ...
    try {
      ...
      const dockerStats = execSync(
        `docker stats ${containerId} --no-stream --format "table {{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}"`,
        { encoding: 'utf8' },
      );
    ...
    const errorRate =
      ((requestStats.errors4xx + requestStats.errors5xx) / requestStats.total) *
      100;
      ...
      services: {
        api: {
          ...
          requests: {
            ...
            errorRate:
              (
                ((requestStats.errors4xx + requestStats.errors5xx) /
                  requestStats.total) *
                100
              ).toFixed(2) + '%',
          },
        ...
        database: {
          ...
          version:
            dbResult[0].database_version?.split(' ')[0] +
            ' ' +
            dbResult[0].database_version?.split(' ')[1],
          connectionCount: parseInt(dbResult[0].connection_count),
          size: dbSize[0].size,
          activeConnections: parseInt(dbSize[0].active_connections),
          statistics: {
            tables: tableStats.length,
            totalInserts: tableStats.reduce(
              (sum, table) => sum + parseInt(table.inserts || 0),
              0,
            ),
            totalUpdates: tableStats.reduce(
              (sum, table) => sum + parseInt(table.updates || 0),
              0,
            ),
            totalDeletes: tableStats.reduce(
              (sum, table) => sum + parseInt(table.deletes || 0),
              0,
            ),
          },
        },
        ...
        summary: {
          ...
          lastAlert:
            alertHistory.length > 0
              ? alertHistory[0].timestamp.toISOString()
              : null,
        },
      },
    });
